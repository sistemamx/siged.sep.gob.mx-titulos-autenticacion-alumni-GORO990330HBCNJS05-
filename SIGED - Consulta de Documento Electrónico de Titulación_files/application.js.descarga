		...
		try {
			...
			function get_filtered_items(target_tag) {
				...
				$items.clone().each(function() {
					console.log('data-tags --> ' + $(this).attr("class"));
...
$(function() {
  ...
  // Auto focus skip to main content AA
  ...
  $('.home_menu').click(function () {
    ...
    $('#subenlaces').removeClass('in').addClass('collapse');
    $('html,body').animate({
...
$('.landing-btn').click(function(event) {
  $('#subenlaces').removeClass('in').addClass('collapse');
});
  ...
  $(".sendEst").click(function () {
    var link = $(this).attr('href');
    var regExp = /^((http|https|ftp):\/\/www.gob.mx)/;
    if (link !== undefined && link.trim() !== '') {
      if (!regExp.test(link)) {
        return confirm('Est\u00E1s saliendo del sitio www.gob.mx');
      }
    }
    var a = $(this)
      .attr("a")
  ...
  $('.registerMail').click(function (event) {
    var page_type = 'Home';
    var organization_name = 'Home';
    ...
    try {
      $.ajax({
        ...
        success: function (data) {
          ...
          $('#responseEmail').html(data.message);
          if (rails_env_production()) {
            if ($('#responseEmail').text().match('El correo fue agregado exitosamente!')) {
              status = "Exitoso";
            } else {
              status = "Error";
            }
            // console.log("Metrica suscripcion "+status+" - " + site);
            ga('set', 'dimension14', pages_group);
            ga('set', 'dimension15', organization_name);
            ga('set', 'dimension16', page_type);
            ga('send', 'event', 'Newsletter', status, site);
        ...
        error: function (data) {
          ...
          $('#responseEmail').html(data.message);
          if (rails_env_production()) {
            // console.log("Metrica suscripcion Error - "+site);
            ga('set', 'dimension14', pages_group);
            ga('set', 'dimension15', organization_name);
            ga('set', 'dimension16', page_type);
            ga('send', 'event', 'Newsletter', 'Error', site);
            ga('set', 'dimension14', null);
            ga('set', 'dimension15', null);
            ga('set', 'dimension16', null);
          }
  ...
  var resize_images = $('.resize-crop-image');

  _.each(resize_images, function (elem) {
    var opts = $(elem).data('size');

    opts.center = true;
    opts.forceResize = true;
    $(elem).resizeAndCrop(opts);
  });
});

$(document).ready(function () {
  // TODO Review if timeout is better
  $(window).load(function () {
    var link = location.href.split('#')[1];
    // console.log('link ??? ' + link );
    var hash = location.hash.replace('#','');
    if(link !== undefined && link === 'contacto') {
  ...
  if ($('#sub_group').length > 0) {
     organization_name = $('#sub_group').val();
  }

  if (rails_env_production()) {
    scroll_page(pages_group, page_type, organization_name, site);

    var c = 0;
    var interval_time = [30, 30, 60, 60];
    var interval = [30, 60, 120, 180];
    (function timer () {
      if (c < 4) {
        setTimeout(function () {
          timer_ga(pages_group ,interval[c - 1], page_type, organization_name);
          timer();
...
function rails_env_production () {
  var rails_env_production = rails_env_prod;
  return rails_env_production;
}

function scroll_page (pages_group, page_type, organization_name, site) {
  var Frequency = 25;
  var _frequency = Frequency;
  var _repentance = 100 / Frequency;
  var _scrollMatrix = new Array();

  for (var ix = 0; ix < _repentance; ix++) {
    _scrollMatrix[ix] = [_frequency, 'false'];
    _frequency = Frequency + _frequency;
  }
  $(document).scroll(function (e) {
    for (var iz = 0; iz < _scrollMatrix.length; iz++) {
      if (($(window).scrollTop() + $(window).height() >= $(document).height() * _scrollMatrix[iz][0] / 100) && (_scrollMatrix[iz][1] == 'false')) {
        _scrollMatrix[iz][1] = 'true';
        /*console.log("PORCENTAJE DE AVANCE: " + _scrollMatrix[iz][0] + "%");
        console.log("PAGES_GROUP : "+pages_group);
        console.log("SUB_GROUP : "+organization_name);
        console.log("PAGE_TYPE : "+page_type);*/

        ga('set', 'dimension14', pages_group);
        ga('set', 'dimension15', organization_name);
        ga('set', 'dimension16', page_type);
        ga('send', 'event', 'Scroll', _scrollMatrix[iz][0]+'%', site, { nonInteraction: true });
        ga('set', 'dimension14', null);
        ga('set', 'dimension15', null);
        ga('set', 'dimension16', null);
      }
...
function timer_ga (pages_group, segundos, page_type, organization_name, site) {
    /*console.log("Metrica Timer - Seconds: "+segundos);
    console.log("PAGES_GROUP : "+pages_group);
    console.log("SUB_GROUP : "+organization_name);
    console.log("PAGE_TYPE : "+page_type);*/
    ...
    ga('set', 'dimension15', organization_name);
    ga('set', 'dimension16', page_type);
    ga('send', 'event', 'Timer', segundos, site, { nonInteraction: true });
    ga('set', 'dimension14', null);
    ga('set', 'dimension15', null);
    ga('set', 'dimension16', null);
};
...
$(document).ready( function() {
    var carga = false;
    $('.errorVideo').fadeOut("slow");
    var type_video = $("#video_type_video").val();

    $('ul.nav-tabs li').each(function(i)
    {
        if(($("#video_type_video").val() == '' || type_video == null) && i == 0) {
            $(this).addClass('active');
            $("#video_type_video").val('youtube');
        }
        if($("#video_type_video").val() == $(this).attr('id')) {
            $(this).addClass('active');
        }
    });
    ...
    $(".video-url").blur(function(e) {
        if ( $(".video-url").val().length != 0 ){
            if ($("#video_type_video").val() == 'youtube'){
                $('.youtube-wrapper').show();
                $('.facebook-wrapper').hide();
                $("#video_default_image").val("https://img.youtube.com/vi/"+videoId+"/mqdefault.jpg");
                  var videoId = $(".video-url").val();
                 var videoTitle = $("#content_video").val();

                  var videoId = $(".video-url").val(); $('.youtube-wrapper').append("<div class='col-xs-12'>" +
                        "<div id='ik_player'></div>" +
                        "<blockquote><h4>" + videoTitle + "</h4></blockquote>" +
                        "</div><input type='hidden' id='videoId' value='" + videoId + "' ");
                    $("#videoIdTitle").val(title);
                ...
                function onYouTubeIframeAPIReady() {
                    var start = $(".video-start").val();


                    var controles = '';
                    if(start> 0){
                        controles = '?start='+start+''
                    }
                ...
                function onPlayerStateChange(event) {
                    switch (event.data) {
                        ...
                        case YT.PlayerState.PLAYING:
                            action = "Play";
                            porcentaje = (player.getCurrentTime() / player.getDuration()) * 100;
                            break;
                    ...
                    if (rails_env_production_v() && porcentaje > 0) {
                        console.log("PORCENTAJE  --> " + porcentaje_str);
            ...
            else if ($("#video_type_video").val() == 'shorts'){
                console.log('short')
                $('.shorts-wrapper').show();
                ...
                $(".video-url").val(videoId);
                $("#video_default_image").val("https://img.youtube.com/vi/"+videoId+"/mqdefault.jpg");

                var urlVideo = 'https://www.googleapis.com/youtube/v3/videos?id=' + videoId + '&key=AIzaSyDVcGSY5yMaYbz9VePw1t8EFfQHBJNw-jM&fields=items(snippet(title))&part=snippet';
                $.getJSON( urlVideo, function(response,status,xhr) {
                    if(typeof response.items[0] === "undefined"){
                        $('.errorVideo').html("<div class='alert alert-danger'>"+
                            "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>"+
                            "<strong>Video no encontrado!!!</strong> Revise por favor..."+
                            "</div>");
                        $("#videoId").focus();
                        $('.errorVideo').fadeIn("fast");
                    }else{
                        $('.errorVideo').fadeOut("slow");
                        var title = response.items[0].snippet.title;

                        $('.shorts-wrapper').html("<div class='row text-center'>"+
                            "<div class='shorts_videos'>"+
            ...
            }else if ($("#video_type_video").val() == 'reels'){
                console.log('reels')
                $('.facebook-wrapper').hide();
                $('.reels-wrapper').show();
                $('.youtube-wrapper').hide();
                ...
                $('.reels-wrapper').html("<iframe src='https://www.instagram.com/reel/"+videoId+"/embed' width='600' height='680' frameborder=0 scrolling='no' allowtransparency='true' allow='encrypted-media'> </iframe>");
                FB.XFBML.parse();

                FB.api(
                    '/'+videoId,
                    'GET',
                    {"fields":"title,picture,format,source", access_token:'2082879135316473|qIkACvTvNDnspCfoYNlEK_QHhQE'},
                    function(response) {
                        if (response!=null && response.id != undefined) {
                            ...
                            response.format.forEach(function(element) {
                                if(element.filter == "native") {
                                    $("#video_default_image").val(element.picture);
                                }
                            });
                        }else{
            ...
            else if ($("#video_type_video").val() == 'facebook'){
                $('.facebook-wrapper').show();
                $('.youtube-wrapper').hide();
                //$('.gobmx-wrapper').hide();

                $('.facebook-wrapper').html("");
                var videoId = $(".video-url").val();

                $('.facebook-wrapper').html("<div class='row text-center'><div class='fb-video' id='fcbk' data-href='https://www.facebook.com/video.php?v=" + videoId + "' data-allowfullscreen='true' data-width='500' data-height='350'></div>" +
                    "</div></div>");
                FB.XFBML.parse();
            ...
            else if ($("#video_type_video").val() == 'gobmx'){
                var stop = $(".video-stop").val();
                var start = $(".video-start").val();
                var videoId = $(".video-url").val();
                $('.gobmx-wrapper').show();
                $('.youtube-wrapper').hide();
                ...
                var controles = '';
                if(start> 0){
                    controles = '?start='+start+'s'
                }else {
                    controles = '?start='+0+'s'
                }
                if (stop > 0){
                    controles = controles+'&stop='+stop+'s'
                }

                $('.gobmx-wrapper').html("<iframe title=\"\" width=\"100%\" height=\"315\" src=\"https://videos.cedn.gob.mx/videos/embed/" + videoId +controles+ "\" frameborder=\"0\" allowfullscreen=\"\" sandbox=\"allow-same-origin allow-scripts allow-popups\">");
    ...
    if($("#video_type_video").val() == 'youtube' || type_video == '' || type_video == null){
        $('.youtube-wrapper').show();
        $('.facebook-wrapper').hide();
        $('.gobmx-wrapper').hide();
    }
    ...
    $('#facebook').on('click', function () {
        //type_video = "facebook";
        $('#video_type_video').val("facebook");
        $(".video-url").val("");
        $('.video-title').val("");
        $('.youtube-wrapper').html("");
        $('.gobmx-wrapper').html("");
    ...
    $('#gobmx').on('click', function () {
        //type_video = "gobmx";
        console.log("CLICK TWITTER...");
        $('#video_type_video').val("gobmx");
        $(".video-url").val("");
        $('.video-title').val("");
        $('.youtube-wrapper').html("");
        $('.facebook-wrapper').html("");
  ...
  $("#btnSaveVideo").click(function(event) {
    event.preventDefault();
    var videoId         = $('#videoId').val();
    var videoIdTitle    = $("#videoIdTitle").val();
    var contentId       = $("#contentId").val();
    var author_id       = $("#author_id").val();
    var organization_id = $("#organization_id").val();

    $.ajax({
      url: '/cms/admin/articles/attach_video',
      type: 'GET',
      dataType: 'html',
      data: { videoId:videoId,
              videoIdTitle:videoIdTitle,
              contentId:contentId,
              author_id:author_id,
      ...
      success: function(data){
        ...
        $("#containerIdVideo").append("<div class='panel panel-light'>"+
                                        "<div class='panel-body' style='background:#dff0d8'>"+
                                            "<button type='button' class='close' data-dismiss='alert' aria-hidden='true' value='"+obj['data']+"'>&times;</button>"+
                                            "<strong> Id: "+obj['response']['videoId']+ " Título: " + obj['response']['videoIdTitle'] +"</strong> Video Guardado"+
                                        "</div>"+
                                      "</div>");
        $("#videoId").val("");
        $("#videoIdTitle").val("");
        ...
        $(".close").bind('click',function(event) {
          var mi_id = $(this).val();
          $.ajax({
            url: '/cms/admin/articles/disattach_video',
            type: 'GET',
            dataType: 'html',
            data: {"idVideo": mi_id},
          })
  ...
  $(".close").bind('click',function(event) {
    ...
    function rails_env_production_v() {
        var rails_env_production = rails_env_prod;
        return rails_env_production;
    }

  $(".video-url").each(function(index, el) {
    var videoId = $(el).val();
    var videoTitle = $(el).attr('title');
    var videoContent  = $(el).attr('accion');

    $.getJSON('https://www.googleapis.com/youtube/v3/videos?id=' + videoId + '&key=none&fields=items(snippet(title))&part=snippet',function(response,status,xhr) {
      if(typeof response.items[0] === "undefined") {
          if($("#type_video").val()=='youtube') {
              $('.errorVideo').html("<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><strong>Video no encontrado!</strong> Revise por favor...</div>");
          }
          if($("#type_video").val()=='facebook'){
              var video_id = $(".video-url").val();
              var porcentaje = 0;
              var porcentaje_str = "";
              var action = "";

              $('.facebook-wrapper').html("<div class='row text-center'><div id='facebook-video' class='fb-video' data-href='https://www.facebook.com/video.php?v=" + video_id + "' data-allowfullscreen='true' data-width='1130' data-height='720'></div>" +
                  "<div class='col-xs-12'><blockquote style='text-align: left !important;'><h4>" + videoTitle + "</h4></blockquote></div>" +
                  "</div></div>");
              $(".facebook-wrapper").show();

              // Seguimiento Google Alalytics.
              var my_video_player;
              setTimeout(function(){
                  FB.XFBML.parse();
                  FB.Event.subscribe('xfbml.ready', function(msg) {
                      ...
                      my_video_player.subscribe('startedPlaying', function (e) {
                          ...
                          console.log("Playing...");
                          action = "Play";
                          porcentaje = (my_video_player.getCurrentPosition() / my_video_player.getDuration()) * 100;
                          console.log("porcentaje --> " + porcentaje);

                          medicion_GA_facebook(porcentaje, action);
                      });
            ...
            /*if ($("#type_video").val()=='gobmx') {
              window.twttr = (function (d, s, id) {
                  ...
                  js.id = id;
                  js.src = "https://platform.gobmx.com/widgets.js";
                  fjs.parentNode.insertBefore(js, fjs);
                  ...
                  return t;
              }(document, "script", "gobmx-wjs"));

              //Wrap event bindings - Wait for async js to load
              //twttr.ready(function (twttr) {
                  //event bindings
                  //console.log("Se cargo tuit...");
                  //twttr.events.bind('click', function (event) {
                     // alert('testing');
                  //});
                  //twttr.events.bind('video_views_25', trackTwitter);
              //});
              ...
              twttr.ready(function (twttr) {
                  //event bindings
                  console.log("Se cargo tuit...");
                  twttr.events.bind('play', trackTwitter);
              });


              $('.gobmx-wrapper').show();
              $('.gobmx-wrapper').html("<div class='row text-center' style='margin-left: 10px;'><div id='twttr-video'>" + "</div>" +
                  "<div class='col-xs-12'><blockquote style='text-align: left !important;'><h4>" + videoTitle + "</h4></blockquote></div>" +
                  "</div>");
              ...
              window.twttr.ready(function(twttr) {
                  twttr.widgets.createVideo(
                      ...
                      function (twttr) {
                          twttr.contentDocument.querySelector(".EmbeddedTweet").style.maxWidth= "100%";
                          twttr.contentDocument.querySelector(".EmbeddedTweet").style.border ="1px";
                          twttr.contentDocument.querySelector(".EmbeddedTweet").style.borderStyle= "solid";
                          twttr.contentDocument.querySelector(".EmbeddedTweet").style.borderColor= "gray";
                          twttr.contentDocument.querySelector(".EmbeddedTweet").style.marginRight= "10px";
                          twttr.contentDocument.querySelector(".Icon.Icon--playCircle").style.fontSize= "75px";

                          twttr.contentDocument.querySelector(".EmbeddedTweet-tweet").remove();
                      }
                  ).then(function(el) {
                  });
              });
        ...
        switch(videoContent) {
            case "video-video":
               // if($("#type_video").val()=='youtube'){
                    ...
                    $('.youtube-wrapper').append("<div class='col-xs-12'>" +
                        "<div id='ik_player' style='width: 100%; height: 100%;'></div>" +
                        "<blockquote><h4>" + videoTitle + "</h4></blockquote>" +
                        "</div><input type='hidden' id='videoId' value='" + video_id + "' ");
                    $("#videoIdTitle").val(title);
                ...
                function onPlayerStateChange(event) {
                    ...
                    if (rails_env_production_v() && porcentaje > 0) {
                        console.log("PORCENTAJE  --> " + porcentaje_str);
                        console.log("ACCION --> " + action);
                        ga('send', 'event', 'Video', porcentaje_str, videoTitle);
          ...
          case "video":
            if($("#video_title").val() == "") {
              $(".video-title").attr("value", title);
              $(".video-url").attr("value", videoId);
            }
            break;
          case "livestream":
            $('.youtube-wrapper').append("<div class='row text-center'>"+
                                          "<div class='col-xs-offset-3 col-xs-6'>"+
                                            "<iframe class='youtube-player video-preview'"+
                                            "src='https://www.youtube.com/embed/" + videoId + "?autoplay=0&rel=0'"+
                                            " width='100%' height='350px' frameborder='0'></iframe>"+
                                          "</div>"+
                                        "</div>");
            break;
          case "actualidad-video":
            $('.youtube-wrapper').append("<div class='text-center'>"+
                                          "<div class='col-xs-4'>"+
                                            "<iframe class='youtube-player video-preview'"+
                                            "src='https://www.youtube.com/embed/" + videoId + "?autoplay=0&rel=0'"+
                                            " width='300px' frameborder='0'></iframe>"+
                                          "</div>"+
                                         "</div>");
            $(".video-title").attr("value", title);
            $(".video-url").attr("value", videoId);
            break;
          default:
            $('.youtube-wrapper').append("<div tabindex='2'>"+
                                          "<iframeid =12 class='youtube-player video-preview'"+
                                            "src='https://www.youtube.com/embed/" + videoId + "?autoplay=0&rel=0&showinfo=0'"+
                                            "width='100%' frameborder='0' style='margin-top: -25px; margin-bottom: 0px; min-height:200px;'>"+
                                          "</iframeid>"+
                                          "<span class='small'>"+videoTitle+"</span>"+
                                        "</div>");
            $("#videoIdTitle").val(title);
        }
      }
   ...
   /* function trackTwitter(intent_event) {
        console.log("INTENT_EVENT --> "+intent_event);
        if (intent_event) {
            var opt_pagePath;
            if (intent_event.target && intent_event.target.nodeName == 'IFRAME') {
                opt_target = extractParamFromUri(intent_event.target.src, 'url');
                console.log("OPT_TARGET ---> "+opt_target);
            }
            //_gaq.push(['_trackSocial', 'gobmx', 'tweet', opt_pagePath]);
        }
    } */

   function medicion_GA_facebook(porcentaje, action){
       var porcentaje_str = "";
       ...
       console.log("PORCENTAJE QA --> " + porcentaje_str);
       console.log("ACCION QA--> " + action);
    ...
    //############################ Live Streaming ############################################################
  $(".video-embeded").blur(function(e) {
    //si el texto es diferente de cero
    if ( $(".video-embeded").val().length != 0 ){
      $(".show_video").show();
      $('.errorVideo').fadeIn("fast");
        $('.youtube-wrapper').html("");
        var embed = $(this).val().toString().trim();
        var exg_exp = /(www.youtube.com\/embed\/[A-Za-z0-9_-]*)/;
        try {
          var idEmbebed = (embed.match(exg_exp)) ? RegExp.$1 : false;
          exg_exp = /([A-Za-z0-9_-]*)$/;
          var videoId = (idEmbebed.match(exg_exp)) ? RegExp.$1 : false;
          $.getJSON('https://www.googleapis.com/youtube/v3/videos?id=' + videoId + '&key=None&fields=items(snippet(title))&part=snippet',function(response,status,xhr) {
            if(typeof response.items[0] === "undefined"){
              $('.errorVideo').html("<div class='alert alert-danger'>"+
                                        "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>"+
              ...
              $('.errorVideo').fadeOut("slow");
              var title = response.items[0].snippet.title;
              $('.youtube-wrapper').html("<div class='row text-center'>"+
                                            "<div class='col-xs-offset-3 col-xs-6'>"+
                                              "<iframe class='youtube-player video-preview'"+
                                              "src='https://www.youtube.com/embed/" + videoId + "?autoplay=0&rel=0'"+
                                              " width='100%' height='350px' frameborder='0'></iframe>"+
                                            "</div>"+
                                          "</div>");
              $(".video_title_embebed").attr("value", title);
            }
          });
        } catch (error2) {
            $(".video_title_embebed").val("");
        }
    }else{
      $('.show_video').hide();
      e.preventDefault();
  ...
  //############################ Boton Programar Live Streaming ############################################
  var programming    = $('#programLiveStream');
  var popup_title    = programming.data('title');
  var live_timestamp = "";

  var setTimestampValues = function() {
      var date = $('#programLiveStream').data('broadcast');
      date = date.split(' ');
      var day = date[0].split('-').reverse().join('/');
      var hour = date[1].split(':')

      $('.broadcast-day').val(day);
      $('.broadcast-hour').val(hour[0] + ":" + hour[1]);

      $('#date-holder').val(day);
      $('.date-hour').val(hour[0]);
      $('.date-minute').val(hour[1]);

      $('#datepicker').empty().removeAttr("class").datepicker({
          onSelect: function(date){
              $('#date-holder').val(date);
              $('.broadcast-day').val(date);
          }
      });

      $('#date-holder').keyup(function(){
          var datepic = $(this).val();
          $('#datepicker').datepicker("setDate", datepic);
          $('.broadcast-day').val(datepic);
      });

      $('.date-hour, .date-minute').change(function(){
          var hour = $('.date-hour').val() + ":" + $('.date-minute').val();
          $('.broadcast-hour').val(hour);
      });

      $("input[type='checkbox']").click(function(){
          $("#button_send_notification").prop("checked", $(this).is(":checked"));
      });

      var datepicked = $('#date-holder').val();
      ...
      '<div class="row">'+
        ...
        '<div class="col-md-6">'+
          '<div class="form-group add-tag-group">'+
            '<input type="text" class="form-control" id="date-holder" placeholder="Fecha">'+
          '</div>'+
          '<div class="row">'+
            '<div class="col-md-6">'+
              '<select class="form-control date-hour">'+
                '<option>00</option>'+
                '<option>01</option>'+
                '<option>02</option>'+
                '<option>03</option>'+
            ...
            '</div>'+
            '<div class="col-md-6">'+
              '<select class="form-control date-minute">'+
                '<option>00</option>'+
                '<option>01</option>'+
                '<option>02</option>'+
                '<option>03</option>'+
          ...
          '<div class="checkbox">'+
            '<label>'+
              '<input type="checkbox">'+
              'Notificarme por correo al publicar'+
            '</label>'+
          '</div>'+
          '<button id="btnProgramLiveStream" type="button" class="btn btn-primary popover-close">Programar</button>'+
        '</div>'+
      '</div>'
  );

  if (typeof($('#programLiveStream').val()) != "undefined") {
    programming.popover({
      html: true,
      title: 'Programar ' + popup_title +'<button type="button" class="close popover-close" aria-hidden="true">&times;</button>',
      content: labelsPopover,
      callback: function() {
        $('.popover').addClass('livestream');
        setTimestampValues();
      }
    });

    programming.on('shown.bs.popover', function () {
      $('.popover-close').click(function(){
        live_timestamp = $('#date-holder').val().split('/').reverse().join('-') + ' ' +$('.date-hour').val() + ":" + $('.date-minute').val() + ' -0600';
        $('#programLiveStream').data('broadcast', live_timestamp);
        //console.log($('#programLiveStream').data('broadcast', live_timestamp));
        setTimestampValues();
        $('.popover-content').parent().hide();
        programming.popover('hide');
      });
    });

    programming.popover('show').popover("hide");



    var fieldVideo = $("#live_stream_youtube_link").val();
    var title      = $("#live_stream_youtube_title").val();
    var streaming  = $("#streaming").attr('id');
    var flag       = false;
    //console.log(flag);

    $("#streaming").click(function(event) {
      flag = true;
      //console.log(flag);
    });

    $("#youtube_live").click(function(event) {
      flag = false;
      //console.log(flag);
    });


    $("#btnProgramLiveStream").click(function(event) {

      if(flag){
        var fieldVideo = $("#live_stream_embeded_link").val();
        var title = $("#live_stream_embeded_title").val();
        //console.log(fieldVideo+"|"+title);
        //console.log(flag)
      }else{
        var fieldVideo = $("#live_stream_youtube_link").val();
        var title = $("#live_stream_youtube_title").val();
      ...
      //var mime_type       = $("").val();
      //var broadcast_at    = $("#programLiveStream").data("broadcast")+". -0600";
      var broadcast_at = $('#date-holder').val().split('/').reverse().join('-') + ' ' +$('.date-hour').val() + ":" + $('.date-minute').val() + ' -0500';
      //console.log(broadcast_at);
      var author_id       = $("#author_id").val();
      var organization_id = $("#organization_id").val();
      var active          = false;
      var youtube_flag    = true;

      //console.log(flag);

      $.ajax({
        url: 'https://localhost:3000/cms/admin/live_streams/programing_live_stream',
        type: 'GET',
        dataType: 'html',
        data: { "fieldVideo":fieldVideo,
                "title":title,
...
// ONLY include when needed; Check policy
// require leaflet.js
// require c3.js
// require cms/admin/mapas.js
...
// PARA VIDEO DE FONDO = require jquery.mb.YTPlayer
